...
public sealed class GuessService
{
	...
	public async Task<GuessResponse?> GetGuessByIdAsync(Guid guessId, CancellationToken token = default)
	{
		Guess? guess = await _guessRepository.GetGuessByIdAsync(guessId, token);
	...
	public async Task<GuessResultResponse?> CreateGuessAsync(GuessRequest request, CancellationToken token = default)
	{
		Place? place = await _placeRepository.GetPlaceByIdAsync(request.PlaceId, token);
		...
		await _guessRepository.InsertGuessAsync(newGuess, token);
